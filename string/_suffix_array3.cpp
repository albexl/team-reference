...
namespace SuffixArray{
    ...
    #define tset(i, b) t[(i)/8]=(b) ? (mask[(i)%8]|t[(i)/8]) : 
        ((~mask[(i)%8])&t[(i)/8])
    ...
    void getBuckets
    (unsigned char *s, int *bkt, int n, int K, int cs, bool end) {
        int i, sum = 0;
        for (i = 0; i <= K; i++)
    ...
    // compute SAl
    void induceSAl
    (unsigned char *t, int *SA, unsigned char *s, int *bkt, 
    ...
    // use a working space (excluding s and SA) of at most 2.25n+O(1)
    // for a constant alphabet
